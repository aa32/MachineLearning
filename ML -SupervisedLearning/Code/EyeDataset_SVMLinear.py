...
pipe = Pipeline(steps = [("scaler",StandardScaler()),("linear_svc",svc)])
...
param_grid = dict(linear_svc__gamma=gammas, linear_svc__C=Cs)
...
print(("time taken to train linear SVC classifier is {:.2f}" "seconds"),format(time() - start))
...
print(("time taken to test linear SVC classifier is {:.2f}" "seconds"),format(time() - start2))
...
print ("Accuracy on training set with linear SVC is %5f",metrics.balanced_accuracy_score(y_train,y_pred_train))
print ("Accuracy with test set with linear SVC is %5f",metrics.balanced_accuracy_score(y_test,y_pred))

print(confusion_matrix(y_test, y_pred))  
print(classification_report(y_test, y_pred))  

plot_learning_curve(model,'EyeState Learning Curve',X_train,y_train,ylim =(0.4,1.1,),cv =10,n_jobs =-1)



GridSearch_table_plot(grid, "linear_svc__gamma" ,negative=False)


GridSearch_table_plot(grid, "linear_svc__C",negative=False)
